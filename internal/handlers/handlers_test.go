...
import (
	...
	// "github.com/bertoxic/bert/internal/driver"
...
var theTests = []struct {
	...
	{"non-existent", "/we-coming/for/you", "GET", http.StatusNotFound},
	//new routes
	{"login", "/user-login", "GET", http.StatusOK},
	{"logout", "/user-logout", "GET", http.StatusOK},
	{"dashboard", "/admin/dashboard", "GET", http.StatusOK},
	{"all res", "/admin/reservations-new", "GET", http.StatusOK},
	{"new res", "/admin/reservations-all", "GET", http.StatusOK},
	{"new res", "/admin/reservation-calender", "GET", http.StatusOK},
...
func TestRepository_Reservation(t *testing.T) {
	...
	sessions.Put(ctx, "reservation", reservation)
	...
	// test case where non-existent room-id (reset everything)
	req, _ = http.NewRequest("GET", "/make-reservation", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	rr = httptest.NewRecorder()
...
func TestRepository_PostReservation(t *testing.T) {
	...
	//testing for invalid input data invalid_start_date
	reqBody = "start_date= invalid-01-01"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2050-01-02")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=John")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "email=john@smith.com")
	...
	//testing for invalid input for invalid  end_date
	reqBody = "start_date=2050-01-01"
	...
	//testing for invalid input for  room_id
	reqBody = "start_date=2090-01-01"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2090-02-01")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=John")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "email=john@smith.com")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "phone=123456789")
	...
	//testing for invalid input for  form validity
	reqBody = "start_date=2090-01-01"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2090-02-01")
	...
	//testing for invalid input for  insertreservation
	reqBody = "start_date=2090-01-01"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2090-02-01")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=John")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
	...
	//testing for invalid input for  room restriction
	reqBody = "start_date=2090-01-01"
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2090-02-01")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=John")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
...
func TestRepository_PostAvailability(t *testing.T) {
	...
	*****************************************/
	...
	postedData = url.Values{}
	postedData.Add("start", "2023-01-01")
	postedData.Add("end", "2023-01-02")
...
func TestRepository_AvailabilityJSON(t *testing.T) {
	...
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end=2090-02-01")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "room_id=1")
	...
	if err != nil {
		t.Error("issues parsing jsondata")
	}
	...
	err = json.NewDecoder(rr.Body).Decode(&n)
	if err != nil {
		t.Error("issues parsing jsondata")
	}
	if n.Message != "unable to connect to the database" {
		t.Error("failed database check test")
	}
...
func TestRepository_Example(t *testing.T) {
	req, _ := http.NewRequest("GET", "/example", nil)
	ctx := getCtx(req)
	req = req.WithContext(ctx)

	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(Repo.Example)
	handler.ServeHTTP(rr, req)
...
func TestRepository_ReservationSummary(t *testing.T) {
	// testing reservation not in session
	...
	handlers := http.HandlerFunc(Repo.ReservationSummary)

	handlers.ServeHTTP(rr, req)
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Reservation handler returned wrong response code: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
	//testing for  reservation in session
	reservation := models.Reservation{
		RoomID: 1,
		Room: models.Room{
			ID:       1,
			RoomName: "General's Quater's",
		},
	}
	...
	rr = httptest.NewRecorder()
	handlers = http.HandlerFunc(Repo.ReservationSummary)
	sessions.Put(req.Context(), "reservation", reservation)
	handlers.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Errorf("Reservation handler returned wrong response code: got %d, wanted %d", rr.Code, http.StatusOK)
	}
...
func TestRepository_ChooseRoom(t *testing.T) {
	//test for the handler to run without errors
	...
	// withoot reservation in session
	...
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	req.RequestURI = "/choose-room/invalid value"
...
func TestRepository_BookRoom(t *testing.T) {
	...
	*****************************************/
	...
	req, _ = http.NewRequest("GET", "/Book-room?s=2050-01-01&e=2050-01-02&id=100", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
...
func TestLogin(t *testing.T) {
	for _, e := range loginTest {
		...
		if rr.Code != e.expectedStatusCode {
			t.Errorf("failed %s expected code %d instead got %d", e.name, e.expectedStatusCode, rr.Code)
		}
		...
		if e.expectedLocation != "" {
			...
			if actualLoc.String() != e.expectedLocation {
				t.Errorf("failed test %s : expected %s, got instead %s", e.name, e.expectedLocation, actualLoc.String())
			}
		}
		...
		if e.expectedHTML!="" {
			...
			if !strings.Contains(html, e.expectedHTML){
				t.Errorf("failed %s : expected to find %s but did not found this%s",e.name, e.expectedHTML,html)
			}
		}
	}
}
...
func getCtx(req *http.Request) context.Context {
	ctx, err := sessions.Load(req.Context(), req.Header.Get("X-Session"))
	if err != nil {
		log.Println(err)
	}
	return ctx
