...
func getRoutes() http.Handler {
	...
	mux.Get("/admin/reservation-calender",Repo.AdminReservationCalender)
	...
	mux.Post("/admin/reservation/{src}/{id}",Repo.AdminModifyReservation)
	mux.Post("/admin/reservation-calender",Repo.AdminPostReservationCalender)
...
func CreateTestTemplateCache() (map[string]*template.Template, error) {
	...
	pages, err := filepath.Glob(fmt.Sprintf("%s/*.page.gtpl", pathToTemplates))
	if err != nil {
		return myCache, err
	}
	...
	for _, page := range pages {
		...
		matches, err := filepath.Glob(fmt.Sprintf("%s/*.layout.gtpl", pathToTemplates))
		if err != nil {
			return myCache, err
		}

		if len(matches) > 0 {
			ts, err = ts.ParseGlob(fmt.Sprintf("%s/*.layout.gtpl", pathToTemplates))
			if err != nil {
				return myCache, err
			}
		}
